...
/**
 * A class that allows {@link com.lib.jsdk.glide.load.resource.transcode.ResourceTranscoder}s to be
...
public class TranscoderRegistry {
  ...
  /**
   * Registers the given {@link com.lib.jsdk.glide.load.resource.transcode.ResourceTranscoder} using
  ...
  public synchronized <Z, R> void register(
          ...
          @NonNull com.lib.jsdk.glide.load.resource.transcode.ResourceTranscoder<Z, R> transcoder) {
    transcoders.add(new Entry<>(decodedClass, transcodedClass, transcoder));
  }

  /**
   * Returns the currently registered
   * {@link com.lib.jsdk.glide.load.resource.transcode.ResourceTranscoder} for the given classes.
   *
  ...
  public synchronized <Z, R> com.lib.jsdk.glide.load.resource.transcode.ResourceTranscoder<Z, R> get(
    ...
    if (transcodedClass.isAssignableFrom(resourceClass)) {
      return (com.lib.jsdk.glide.load.resource.transcode.ResourceTranscoder<Z, R>) UnitTranscoder.get();
    }
    for (Entry<?, ?> entry : transcoders) {
      if (entry.handles(resourceClass, transcodedClass)) {
        return (com.lib.jsdk.glide.load.resource.transcode.ResourceTranscoder<Z, R>) entry.transcoder;
      }
    }
